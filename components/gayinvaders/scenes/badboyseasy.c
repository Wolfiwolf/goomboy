...
{
	const asset_info_t *ass = wd_get_asset_info(ASSET_TYPE_BADBOYSEASY);
	game_object_t go;
	render_obj_t ro;
	uint16_t *buff;
	int x, y;
	...
	for (x = 0; x < ass->w; x += ro.w) {
		for (y = 0; y < ass->h; y += ro.h) {
			go.x = x+(float)ro.w/2;
			go.y = y+(float)ro.h/2;
			wd_read_asset_direct(ASSET_TYPE_BADBOYSEASY, buff, x, y, ro.w, ro.h);
			ro.buff = buff;
			renderer_render(&ro);
		}
	}
...
static scene_t _badboyseasy_scene = {
	.init = _init,
